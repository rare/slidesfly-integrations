...
This page records evidence candidates for the OpenSSF Best Practices passing-level
self-assessment for this public repository. No OpenSSF Best Practices badge has been earned yet.
The official application remains the source of truth for every criterion and for any future badge
status.
...
## Passing-level evidence candidates

| Criteria area | Current public evidence | Readiness boundary |
|---|---|---|
| Project description and use | [`README.md`](../README.md) explains the problem, scope, install paths, examples, and current limitations. | Candidate evidence; the official form must accept it. |
...
| Warning and static analysis | [CodeQL](../.github/workflows/codeql.yml), Node validation scripts, workflow pin validation, build failures, and npm audit checks run publicly. | Tool applicability and any N/A answer must be justified in the official form. |
| Secure delivery | GitHub and npm use HTTPS; releases publish SHA-256/SHA-512 checksum files, the verifier recomputes registry artifact hashes, and the manual [release-attestation workflow](release-attestations.md) can publish a Sigstore-signed in-toto release attestation after strict asset verification. | The workflow is readiness evidence only until a release contains a bundle that consumers can verify. It does not claim SLSA build provenance for npm packages built elsewhere. |
...
3. Answer every MUST, SHOULD, and SUGGESTED criterion with criterion-specific evidence or a truthful
   justification.
4. Resolve any criterion that the official validator rejects; do not weaken branch protection,
   fabricate contributors, or create empty releases to improve a score.
5. Add the official badge only after the project page reports that the passing level is earned.
